...
def Experiment(data_list, root, tau_list, pred_list, num_exp, result=False):
    print('running...')
        ...
        for tau in tau_list:
            ...
            for pred_steps in pred_list:
                for exp in range(num_exp):
                    ...
                    dataloader, test_dataset = gbp.process_data_normalize_GRU(x=x, edge_fixed=edge_fixed,
                                                                              edge_attr=edge_attr, T=T, tau=tau,
                                                                              num_nodes=num_nodes,
                                                                              num_edges=num_edges,
                                                                              batch_size=batch_size,
                                                                              num_training=num_training,
                                                                              pred_steps=pred_steps, device=device)
                    if torch.cuda.is_available():
                        dataloader = fdl.CudaDataLoader(dataloader, device=0)
                    model = mp.EncoderDecoder_GRU(tau,pred_steps= pred_steps,num_nodes=num_nodes).to(device)
                    #model = mp.EncoderDecoder_LSTM(tau,pred_steps=pred_steps,num_nodes=num_nodes).to(device)
                    ...
                    train_loss_collections, val_loss_collections,epoch_time ,ev_list,y_hat_test = tp.train_for_GRU_Cudaloader(100, optimizer=updater,
                    ...
                    df_new = pd.DataFrame(data=[
                        [str(model.__class__.__name__), tau, pred_steps, ev_list[0], ev_list[1], ev_list[2], ev_list[3],
                         ev_list[4], ev_list[5]]],
                    ...
                    path = './train/' + str(model.__class__.__name__) + '/' + str(T) + '_' + str(T_train) + '/' + str(
                        tau) + '_steps_for_pred' + str(pred_steps) + '_result/'
